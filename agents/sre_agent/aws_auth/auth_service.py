...
class AWSAuthService:
        ...
        if self._sts_client is None:
            try:
                ...
                if self.config.default_profile:
                    ...
                    self._sts_client = session.client("sts", region_name=self.config.default_region)
                else:
                    self._sts_client = boto3.client("sts", region_name=self.config.default_region)
                ...
                raise AWSAuthError(f"Failed to initialize STS client: {e}", original_error=e)
    ...
    async def _assume_role(self, role_config: RoleConfig) -> AWSCredentials:
        ...
        logger.info(f"Assuming role: {role_config.role_arn} with session: {session_name}")
        ...
        try:
            ...
            raise AWSAuthError(f"Unexpected error during role assumption: {e}", original_error=e)
        ...
        **client_kwargs,
        ...
        try:
            ...
            if role_name:
                ...
                logger.debug(f"Creating {service} client with assumed role: {role_name}")
                ...
                if self.config.default_profile:
                    session = boto3.Session(profile_name=self.config.default_profile)
                    return session.client(service, region_name=target_region, **client_kwargs)
                else:
                    return boto3.client(service, region_name=target_region, **client_kwargs)
    ...
    async def test_credentials(self, role_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Credential test successful for role: {role_name or 'default'}")
            return {
            ...
            logger.error(f"Credential test failed for role {role_name or 'default'}: {e}")
        ...
        for role_name, (credentials, cache_time) in self._credential_cache.items():
            try:
                ...
                cache_info[role_name] = {
                    "cached_at": datetime.fromtimestamp(cache_time, tz=timezone.utc).isoformat(),
