...
class RoleConfig(BaseModel):
    ...
    def validate_role_arn(cls, v):
        ...
        if not v.startswith("arn:aws:iam::"):
            raise ValueError("role_arn must be a valid IAM role ARN starting with arn:aws:iam::")
        if ":role/" not in v:
    ...
    def from_env(cls) -> "AWSAuthConfig":
        ...
        if cache_ttl := os.getenv("AWS_AUTH_CACHE_TTL"):
            try:
                ...
                logger.warning(f"Invalid AWS_AUTH_CACHE_TTL value: {cache_ttl}, using default")
        ...
        if default_role_arn and default_account_id:
            ...
            config_data.setdefault("roles", {})[default_role_name] = RoleConfig(
                ...
                role_session_name=os.getenv("AWS_AUTH_DEFAULT_SESSION_NAME", "SREBotSession"),
