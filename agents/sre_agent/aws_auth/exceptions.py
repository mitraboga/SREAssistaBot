...
class AssumeRoleError(AWSAuthError):
    ...
    def __init__(
        ...
        """
        error_code = (
            f"ASSUME_ROLE_FAILED_{aws_error_code}" if aws_error_code else "ASSUME_ROLE_FAILED"
        )
...
def create_auth_error_from_boto_error(boto_error, context: str = "AWS operation") -> AWSAuthError:
    """
    ...
    if isinstance(boto_error, ClientError):
        ...
        error_message = boto_error.response.get("Error", {}).get("Message", str(boto_error))
