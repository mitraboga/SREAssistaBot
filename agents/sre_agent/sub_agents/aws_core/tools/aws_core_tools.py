...
async def list_ec2_instances(
    ...
    try:
        auth_service = get_auth_service()
        ec2_client = await auth_service.get_client("ec2", role_name=role_name, region=region)
...
async def list_rds_instances(
    ...
    try:
        auth_service = get_auth_service()
        rds_client = await auth_service.get_client("rds", role_name=role_name, region=region)
...
async def get_aws_regions(role_name: Optional[str] = None, service: str = "ec2") -> Dict[str, Any]:
    """
...
async def test_aws_connectivity(role_name: Optional[str] = None) -> Dict[str, Any]:
    ...
    success_count = sum(1 for test in test_results["tests"].values() if test["status"] == "success")
    total_count = len(test_results["tests"])

    test_results["overall_status"] = (
        "success" if success_count == total_count else "partial" if success_count > 0 else "error"
    )
