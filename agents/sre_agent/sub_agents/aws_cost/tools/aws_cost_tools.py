    ...
    if _cost_explorer is None:
        try:
            if _auth_service and _auth_role_name:
                ...
                _cost_explorer = await _auth_service.get_client("ce", role_name=_auth_role_name)
            else:
                # Fallback to default credentials (existing behavior)
                logger.debug("Creating Cost Explorer client with default credentials")
                _cost_explorer = boto3.client("ce")
...
def configure_aws_auth(auth_service=None, role_name: Optional[str] = None):
    ...
    if auth_service and role_name:
        logger.info(f"AWS Cost tools configured to use auth service with role: {role_name}")
    else:
...
async def get_cost_excluding_services(
    ...
    filter_expression = {"Not": {"Dimensions": {"Key": "SERVICE", "Values": excluded_services}}}
    ...
    if result["status"] == "success":
        ...
        for period in time_periods:
            ...
            if "Total" not in period or not period["Total"]:
                logger.warning(f"No Total metrics found for period {period_start} to {period_end}")
                total_by_period.append({"start": period_start, "end": period_end, "total": 0})
                continue
            ...
            total_by_period.append({"start": period_start, "end": period_end, "total": total})
        ...
        for i in range(1, len(total_by_period)):
            ...
            trend_data.append(
                {
                    "period": total_by_period[i]["start"] + " to " + total_by_period[i]["end"],
        ...
        if result["status"] == "success":
            ...
            for period in result["data"]["ResultsByTime"]:
                for group in period["Groups"]:
                    account_info = group["Keys"][0]  # Format: "account_id (account_name)"
                    ...
                    if cost > most_expensive["cost"]:
                        ...
                        account_id = parts[0]
                        account_name = parts[1].strip("()") if len(parts) > 1 else "Unknown"
...
async def get_cost_by_service(start_date: str, end_date: str, granularity: str = "MONTHLY") -> Dict:
    """
...
async def get_digital_cost_for_month(
    ...
    return await get_monthly_cost(year=year, month=month, filter_expression=filter_expression)
